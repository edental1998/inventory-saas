...
import {
  getDailySalesTrend,
  getTopProductsForBranch,
  getWasteVsSalesForBranch,
  getEstimatedVsActualForBranch,
  getWasteReasonBreakdownForBranch,
} from "@/lib/data/sales-analytics";
import { SalesImportForm } from "@/components/sales/SalesImportForm";
...
export default async function BranchSalesPage({
  ...
  const [trend, topProducts, waste, estimatedVsActual, wasteReasons] = await Promise.all([
    getDailySalesTrend(branch.id),
    getTopProductsForBranch(branch.id),
    getWasteVsSalesForBranch(branch.id),
    getEstimatedVsActualForBranch(branch.id),
    getWasteReasonBreakdownForBranch(branch.id),
  ]);
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <SalesImportForm branchId={branch.id} />
